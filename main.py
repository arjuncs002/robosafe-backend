...
from db import (
    ...
    MapFlag, RoverPosition, DriveMode, AutoCommand
)
...
async def toggle_mmwave(request: Request, payload: dict | None = None):
    ...
    if not enabled:
        MMWAVE_STATE.update({
            "status": "SENSOR DISABLED",
            "last_presence": 0, "energy_delta": 0,
            "distance": 0, "energy_min": 0, "energy_max": 0,
            "respiration_detected": False,
        })
...
async def get_latest_command():
    if DRIVE_MODE_STATE["mode"] == "AUTO":
        return {"command": AUTO_CMD_STATE["command"], "ts": AUTO_CMD_STATE["ts"], "source": "auto"}
    return {"command": ROVER_STATE["last_command"], "ts": ROVER_STATE["last_command_ts"], "source": "manual"}
...
async def post_alert(payload: dict | None = None):
    """Jetson calls this when rover arrives at human — triggers siren + mmWave popup on website."""
    if payload is None:
        payload = {}
    ALERT_STATE["type"]   = payload.get("type", "siren")
    ALERT_STATE["ts"]     = time.time()
    ALERT_STATE["active"] = True
    print("[Alert] Siren triggered")
    return {"ok": True}

@app.post("/api/alert/clear")
async def clear_alert(request: Request):
    """Website calls this when operator dismisses popup."""
    verify_token(request)
